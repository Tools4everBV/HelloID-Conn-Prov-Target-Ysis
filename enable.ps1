...
try {
    ...
    if ($dryRun -eq $true) {
        Write-Warning "[DryRun] Enable YsisV2 account for: [$($p.DisplayName)] will be executed during enforcement"
    }

    # Process
    if (-not($dryRun -eq $true)) {
        Write-Verbose "Enabling YsisV2 account with accountReference: [$($aRef.Id)]"
