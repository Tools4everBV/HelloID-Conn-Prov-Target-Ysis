...
#The complete account is needed to update the active field to False due of the PUT method is used by the Ysis webservice. Otherwise, you clear the other fields.
$account = [PSCustomObject]@{
    ...
    'urn:ietf:params:scim:schemas:extension:ysis:1.0:User' = [PSCustomObject]@{
        ...
        profession = [PSCustomObject]@{
            code        = ''
            meaning     = ''
            explanation = ''  # needs to be on of COD878-DBCO, see https://www.vektis.nl/standaardisatie/codelijsten/COD878-DBCO
        }
    }
}
...
try {
    ...
    if ($dryRun) {
        $auditMessage = "Ysis account will be disabled for: $($p.DisplayName)"
    }

    if (-not ($dryRun -eq $true)) {
        # Not sure if this check must take place here.
        # if ( $aRef.Discipline -ne $account.userType ) {
        #     Write-Verbose "The users displine is changed from [$($aref.Discipline)] to [$discipline]. We cannot disable the account."
        #     Write-Verbose 'Take the hand-operated steps to solve the issue.'
        #     throw "The users displine is changed from [$($aref.Discipline)] to [$discipline]. We cannot disable the account."
        # }

        Write-Verbose "Disabling account based on IAM_ID $($account.id)"
        ...
        $auditMessage = "Disabling  account with username $($account.userName) and iam-id  $($account.id)"
    }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $errorMessage = "Could not disable account, Error: $($errorObj.ErrorMessage)"
    } else {
        $errorMessage = "Could not disable account, Error: $($ex.Exception.Message)"
    }
