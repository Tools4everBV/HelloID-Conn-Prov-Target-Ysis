...
try {
    ...
    Write-Verbose "Verifying if a YsisV2 account for [$($p.DisplayName)] exists"
    try {
        $splatParams = @{
    ...
    if ($responseUser){
        $action = 'Found'
        $dryRunMessage = "Disable YsisV2 account for: [$($p.DisplayName)] will be executed during enforcement"
    ...
    if (-not($dryRun -eq $true)) {
        switch ($action){
            'Found' {
                Write-Verbose "Disable YsisV2 account with accountReference: [$($aRef.Id)]"
